...
TOOLS = [
    ...
    {"name": "notes_create", "description": "Create a note for the user. Optional project_id links the note to a project so it appears under that project.", "inputSchema": {"type": "object", "properties": {"title": {"type": "string"}, "content": {"type": "string"}, "category": {"type": "string"}, "project_id": {"type": "string"}}, "required": ["title"]}},
    ...
    {"name": "notes_update", "description": "Update one or more fields of an existing note by id (title/content/category/project_id). To APPEND to an existing note, call notes_list first to get its current content+id, combine with the new text, then send the FULL combined content here. ALWAYS use this for 'add to / update a note' — never create a new note for that.", "inputSchema": {"type": "object", "properties": {"id": {"type": "string"}, "title": {"type": "string"}, "content": {"type": "string"}, "category": {"type": "string"}, "project_id": {"type": "string"}}, "required": ["id"]}},
...
def dispatch(name, args):
    ...
    if name == "notes_create":
        return call("POST", "/notes", {k: a.get(k) for k in ("title", "content", "category", "project_id") if k in a})
    if name == "notes_delete":
        return call("DELETE", "/notes/%s" % a.get("id"))
    if name == "notes_update":
        return call("PUT", "/notes/%s" % a.get("id"), {k: a.get(k) for k in ("title", "content", "category", "project_id") if k in a})
